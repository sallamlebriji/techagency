...
const defaultAudiences = [
  { title: 'PME / TPE', image: 'https://images.unsplash.com/photo-1551434678-e076c223a692?auto=format&fit=crop&w=500&q=80', className: 'left-[82%] top-[43%]' },
  { title: 'Startups', image: 'https://images.unsplash.com/photo-1531482615713-2afd69097998?auto=format&fit=crop&w=500&q=80', className: 'left-[68%] top-[78%]' },
  { title: 'Ecoles', image: 'https://images.unsplash.com/photo-1581090700227-1e37b190418e?auto=format&fit=crop&w=500&q=80', className: 'left-[36%] top-[84%]' },
  { title: 'Commerce', image: 'https://images.unsplash.com/photo-1516321318423-f06f85e504b3?auto=format&fit=crop&w=500&q=80', className: 'left-[18%] top-[55%]' },
  { title: 'Cabinets', image: 'https://images.unsplash.com/photo-1497366811353-6870744d04b2?auto=format&fit=crop&w=500&q=80', className: 'left-[30%] top-[24%]' },
  { title: 'Services', image: 'https://images.unsplash.com/photo-1558494949-ef010cbdcc31?auto=format&fit=crop&w=500&q=80', className: 'left-[58%] top-[17%]' },
];
...
function parseAudiences(value) {
  ...
  return lines.map((line, index) => {
    ...
    return {
      ...
      className: defaultAudiences[index % defaultAudiences.length].className,
    };
  });
}
      ...
      <div className="container-shell">
        ...
        >
          <div className="absolute left-1/2 top-1/2 z-20 flex -translate-x-1/2 -translate-y-1/2 items-center gap-3 rounded-lg border border-slate-200 bg-white/95 px-7 py-5 shadow-premium backdrop-blur">
            ...
            <span>
              <span className="block font-display text-3xl font-bold text-navy">{settings.agencyName || 'TechAgency'}</span>
              <span className="text-xs font-extrabold uppercase text-cyan">{settings.tagline || 'Software & AI Studio'}</span>
            </span>
          </div>

          <div className="orbit-ring absolute inset-0">
            {audiences.map((audience) => (
              <div key={audience.title} className={`orbit-item absolute ${audience.className} z-10 flex w-32 flex-col items-center`}>
        ...
        <div className="mt-10 grid gap-4 md:hidden">
          {audiences.map((audience) => (
            <div key={audience.title} className="flex items-center gap-4 rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
              <img src={audience.image} alt={audience.title} loading="lazy" className="h-16 w-16 rounded-lg object-cover" />
              <div>
                <p className="font-display text-lg font-bold text-navy">{audience.title}</p>
                <p className="mt-1 text-sm text-slate-600">{settings.audiencesCardText || 'Solution digitale adaptee a votre activite.'}</p>
              </div>
            </div>
          ))}
        </div>
